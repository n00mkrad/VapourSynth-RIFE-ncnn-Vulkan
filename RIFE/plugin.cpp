...
void motionVectorGridPackBlob(const std::vector<MVToolsVector>& vectors, const bool valid,
                              const MVAnalysisData& analysisData, std::vector<char>& blob,
                              MotionVectorFrameStats* const stats, const bool includeSadStats,
                              const bool includeMotionStats) {
    packMotionVectorBlob(vectors, valid, analysisData, blob, stats, includeSadStats, includeMotionStats);
}

void motionVectorGridSetProperties(VSMap* props, const MVAnalysisData& analysisData,
                                   const char* vectorBlob, const int vectorBlobSize,
                                   const MotionVectorFrameStats& stats, const bool includeSadStats,
                                   const bool includeMotionStats, const VSAPI* vsapi) {
    setMotionVectorProperties(props, analysisData, vectorBlob, vectorBlobSize, stats,
                              includeSadStats, includeMotionStats, vsapi);
}

int motionVectorGridClampComponent(const int value, const int pel, const int blockCoord,
                                   const int blockSize, const int size, const int padding) noexcept {
    return clampMotionVectorComponent(value, pel, blockCoord, blockSize, size, padding);
}

char* motionVectorGridPrepareBlob(const size_t vectorCount, const bool valid, std::vector<char>& blob) {
...
static void packMotionVectorBlobDirect(const size_t vectorCount, const bool valid, const int64_t invalidSad,
    ...
    auto* output = motionVectorGridPrepareBlob(vectorCount, valid, blob);
...
static void buildMotionVectorBlobsFromGpuVectors(const RIFEGpuMotionVector* gpuVectors,
    ...
    if (!includeSadStats && !includeMotionStats) {
        ...
        const auto packDirection = [&](const RIFEGpuMotionVector* source, std::vector<char>& blob) {
            ...
            auto* output = motionVectorGridPrepareBlob(vectorCount, true, blob);
...
VS_EXTERNAL_API(void) VapourSynthPluginInit2(VSPlugin* plugin, const VSPLUGINAPI* vspapi) {
    ...
    registerCropGrid(plugin, vspapi);
    registerScaleGrid(plugin, vspapi);
